...
A terminal flowline (outlet) is identified by an explicit rule: a row whose `toid` value is not present in the `id` column. The actual value can be anything — `0` (the canonical numeric default), `""` (the canonical character default), `NA`, an arbitrary "no downstream" sentinel from another data source, or a unique downstream identifier per outlet. `hy()` replaces `NA` `toid` values with the canonical sentinel so that user code comparing `toid` to `0` or `""` works as expected, but downstream functions detect outlets by the rule (`!toid %in% id`) and do not depend on the canonical value. Unique-per-outlet identifiers are preserved through the pipeline, which is useful when outlets must remain individually addressable.
