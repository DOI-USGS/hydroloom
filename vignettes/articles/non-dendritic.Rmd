...
`vignette("hydroloom")` and the [topological sort based network attributes article](https://doi-usgs.github.io/hydroloom/articles/advanced_network.html) cover the basics of network topology representation and the attributes that build on a strictly dendritic network. This vignette extends those topics to the `hydroloom` functionality that supports non-dendritic networks.
...
the [network navigation article](https://doi-usgs.github.io/hydroloom/articles/network_navigation.html).
...
nondend <- add_toids(hy(flow_net_with_div), return_dendritic = FALSE)
