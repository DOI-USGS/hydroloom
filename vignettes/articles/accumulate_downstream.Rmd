...
`accumulate_downstream()` provides three modes for this operation, each handling diversions (non-dendritic flow splits) differently. This article demonstrates all three modes using the sample data included with `hydroloom`. For background on non-dendritic network concepts, see the [non-dendritic networks article](https://doi-usgs.github.io/hydroloom/articles/non-dendritic.html).
...
```{r}
suppressWarnings(
ppt7100 <- hydrogeofetch::get_catchment_characteristics(c("CAT_PPT7100_ANN", "TOT_PPT7100_ANN"), ids = x$COMID)
)
