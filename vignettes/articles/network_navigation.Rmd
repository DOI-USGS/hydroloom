...
2. `navigate_hydro_network()` requires the nhdplus representation of a hydrologic network (see the [topological sort based network attributes article](https://doi-usgs.github.io/hydroloom/articles/advanced_network.html)) and returns ids encountered along the requested navigation as a single vector.
...
hydrogeofetch::plot_nhdplus(bbox = sf::st_bbox(hy_net), plot_config = pc)
...
par(mar = c(0, 0, 0, 0))
hydrogeofetch::plot_nhdplus(bbox = sf::st_bbox(different_upmain), plot_config = pc)
