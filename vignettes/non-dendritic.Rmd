...
`vignette("hydroloom")` andd `vignette("advanced_network")` talks about the basics of network topology representation and attributes that build on a strictly dendritic network. This vignette expands those topics by describing `hydroloom` functionality that supports non-dendritic networks.
...
flow_net <- flow_net |>
  ...
  left_join(st_drop_geometry(flow_net), by = "COMID")
