...
If `outlets` is a vector, all combinations
([combn](https://rdrr.io/r/utils/combn.html)) of the given `outlets`
will be produced. If `outlets` is a list, it must be of length 2 (froms,
tos). Recycling is performed via
[expand.grid](https://rdrr.io/r/base/expand.grid.html) if child vector
lengths do not match. `outlets` may also be a 2-column dataframe.
